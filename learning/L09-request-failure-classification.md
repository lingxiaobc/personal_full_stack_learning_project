...
本轮页面集成复测已经覆盖：错端口导致无可用 Response、CORS 预检通过、200 文本响应解析失败、200 JSON 正常响应，以及 400 JSON 错误响应。用户能够根据页面、Network、Console 和后端终端把同一请求在各层的状态对应起来。最后的 400 JSON 字段失配实验是在 AI 先提示实验字段后完成，因此保留为练习证据；下一次用新的等价变式做无提示验收。
...
| 页面发送 `{ userName: "lenox" }` 到 `/api/hello` | 后端收到请求并返回 `400` JSON；前端出现 `[前端 3]`、`[前端 4]`、错误响应分支，不进入 `catch`；页面显示 `name is required`。 | HTTP 400 仍然有 Response；JSON 解析成功与 HTTP 业务成功是两件事。此项为有提示练习证据，待无提示等价变式验收。 |
...
- 用新的、未提前透露根因的等价变式完成一次无提示验收；当前主要页面证据已经齐全，但最后字段失配实验属于有提示练习。
- 无提示验收通过后，再决定是否将 L09 标记为正式通过并进入 L10；本轮先暂停消化，不提前进入下一课。

## 暂定掌握情况

本轮前半段属于有讲解和关键提示的练习；页面集成证据已经补齐，正常、失败和解析异常路径均有截图或终端日志。最后字段失配变式由 AI 先指出实验字段后执行，因此无提示迁移项仍待复测。

暂定评分：解释 2/2，证据 2/2，迁移 1/2，验证与恢复 2/2。评分依据是用户能结合页面、Network、Console 和后端终端复述数据流，并在故障后恢复正常 JSON；迁移分暂不满分，直到完成未透露根因的等价题。

## 下一步

1. 暂停消化本轮数据流，保留当前恢复后的正常代码。
2. 下一次先做一题不提前透露根因的等价变式，用户先给检查顺序和预测，再由 AI 执行最小修改。
3. 无提示变式通过后，完成 L09 正式复盘与状态更新，再决定是否进入 L10。
